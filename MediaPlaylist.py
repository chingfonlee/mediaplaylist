...
def safe_import_name(name):
    name = Path(urllib.parse.unquote(name or '')).name.strip()
    if not name:
        name = 'dropped-file'
    cleaned = ''.join(ch if ch not in '<>:"/\\|?*\x00' else '_' for ch in name)
    cleaned = cleaned.strip(' .') or 'dropped-file'
    return cleaned[:180]

def unique_import_path(name):
    IMPORT_DIR.mkdir(parents=True, exist_ok=True)
    base = safe_import_name(name)
    stem = Path(base).stem or 'dropped-file'
    suffix = Path(base).suffix
    dst = IMPORT_DIR / base
    while dst.exists():
        dst = IMPORT_DIR / f'{stem}-{secrets.token_hex(3)}{suffix}'
    return dst
...
class H(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if u.path == '/api/import-dropped-file':
            q = urllib.parse.parse_qs(u.query)
            ...
            try:
                ...
                try:
                    ...
                    pass
                self.send_json({'ok': False, 'err': str(e)}, 500)
            return
...
.logo{
  width:38px;height:38px;border-radius:12px;position:relative;flex-shrink:0;
  display:inline-flex;align-items:center;justify-content:center;
  background:linear-gradient(145deg,#f8f8f7 0%,#eceef1 48%,#dde1e6 100%);
  box-shadow:inset 0 1px 0 rgba(255,255,255,.92),0 6px 16px rgba(73,78,88,.14);
  overflow:hidden;
}
.logo::before{
  content:"";position:absolute;left:7px;right:7px;top:9px;height:8px;
  border-radius:3px;background:linear-gradient(90deg,#8f8fa3 0%,#b9bbc8 100%);
  transform:rotate(-10deg);transform-origin:left center;
  box-shadow:
    7px 0 0 rgba(255,255,255,.46),
    14px 0 0 rgba(255,255,255,.38);
}
.logo::after{
  content:"";position:absolute;left:9px;right:9px;bottom:8px;height:17px;
  border-radius:4px;background:linear-gradient(180deg,#a7a9b7 0%,#777d8f 100%);
  box-shadow:inset 0 1px 0 rgba(255,255,255,.42);
}
.logo-play{
  position:relative;z-index:2;width:0;height:0;margin-left:2px;margin-top:8px;
  border-top:6px solid transparent;border-bottom:6px solid transparent;
  border-left:9px solid rgba(255,255,255,.92);
  filter:drop-shadow(0 1px 1px rgba(62,67,78,.18));
}
...
.kcard.kcard-drop-target::after{
  ...
  color:var(--accent);font-size:14px;font-weight:850;
}
...
.kc-pick-btn{
  width:42px;height:42px;padding:0;border-radius:14px;font-size:18px;white-space:nowrap;flex-shrink:0;
...
.kc-pick-btn:disabled .kc-file-icon{opacity:.45}
.kc-file-icon{
  position:relative;width:22px;height:27px;display:inline-block;
  border-radius:6px;
  background:linear-gradient(180deg,#f7f8fa 0%,#e4e7eb 100%);
  border:1px solid rgba(115,121,132,.28);
...
.kc-file-icon::before{
  ...
  border-left:1px solid rgba(115,121,132,.22);
  border-bottom:1px solid rgba(115,121,132,.18);
}
.kc-file-icon::after{
  content:"";position:absolute;left:5px;right:5px;top:13px;height:2px;
  border-radius:999px;background:#a6adb7;
  box-shadow:0 5px 0 #c0c5cc;
}
.kc-copy-btn{
  width:42px;height:42px;padding:0;border-radius:14px;font-size:17px;white-space:nowrap;flex-shrink:0;
  background:#fff;border:1px solid rgba(0,0,0,.10);cursor:pointer;font-family:inherit;
  transition:background .12s;box-shadow:none;color:var(--muted);
  display:inline-flex;align-items:center;justify-content:center;
}
.kc-copy-btn:hover{background:var(--surf2);color:var(--text);transform:none}
...
function renderCardMode() {
  ...
  const cards = kcData.slice(0, kcCount).map((card, i) => {
    ...
    return '<div class="' + cls + '" onclick="openKCard(' + i + ')" ' +
      'ondragenter="handleKCardDragEnter(event)" ' +
      'ondragover="handleKCardDragOver(event)" ' +
      'ondragleave="handleKCardDragLeave(event)" ' +
      'ondrop="dropFileOnKCard(event,' + i + ')">' +
      thumbHtml + emptyHtml +
...
function decodeFileUri(uri) {
  ...
  const fileMatch = s.match(/file:\/\/[^\s"'<>]+/i);
  if (fileMatch) s = fileMatch[0];
  ...
  try {
    ...
    return decodeURIComponent(s.replace(/^file:\/+/i, '')).replace(/\//g, '\\');
  }
}

function pathLooksLocal(path) {
  const p = String(path || '').trim();
  return /^[A-Za-z]:[\\/]/.test(p) || /^\\\\[^\\]+\\[^\\]+/.test(p);
}

function extractDroppedFilePath(dt) {
  if (!dt) return '';
  const textTypes = ['text/uri-list', 'text/plain', 'text/html'];
  for (const type of textTypes) {
    try {
      const raw = dt.getData(type);
      if (!raw) continue;
      const parts = raw.split(/\r?\n/).map(decodeFileUri).filter(Boolean);
      const found = parts.find(pathLooksLocal);
      if (found) return found;
      const htmlUri = raw.match(/file:\/\/[^\s"'<>]+/i);
      if (htmlUri) {
        const p = decodeFileUri(htmlUri[0]);
        if (pathLooksLocal(p)) return p;
      }
    } catch {}
  }
  const files = Array.from(dt.files || []);
  for (const f of files) {
    const p = f.path || f.mozFullPath || '';
    if (pathLooksLocal(p)) return p;
  }
  if (dt.items) {
    for (const item of Array.from(dt.items)) {
      try {
        ...
        const p = f && (f.path || f.mozFullPath || '');
        if (pathLooksLocal(p)) return p;
      } catch {}
    }
  }
...
function firstDroppedFile(dt) {
  ...
  if (dt && dt.items) {
    for (const item of Array.from(dt.items)) {
      try {
        const f = item.kind === 'file' && item.getAsFile ? item.getAsFile() : null;
        if (f) return f;
      } catch {}
    }
  }
...
async function importDroppedFile(file) {
  if (!file) return null;
  const r = await fetch('/api/import-dropped-file?name=' + encodeURIComponent(file.name || 'dropped-file'), {
    method: 'POST',
...
function handleKCardDragEnter(e) {
  if (!isExternalFileDrag(e)) return;
  e.preventDefault();
  e.stopPropagation();
  e.currentTarget.classList.add('kcard-drop-target');
}

function handleKCardDragOver(e) {
  if (!isExternalFileDrag(e)) return;
  e.preventDefault();
  e.stopPropagation();
...
function handleKCardDragLeave(e) {
  if (!isExternalFileDrag(e)) return;
  if (e.relatedTarget && e.currentTarget.contains(e.relatedTarget)) return;
  e.currentTarget.classList.remove('kcard-drop-target');
}

async function dropFileOnKCard(e, index) {
  if (!isExternalFileDrag(e)) return;
  e.preventDefault();
  e.stopPropagation();
  e.currentTarget.classList.remove('kcard-drop-target');

  let path = extractDroppedFilePath(e.dataTransfer);
  if (!path) {
  ...
  while (kcData.length <= index) {
    kcData.push({id: kcData.length, title:'', file:'', thumbnail:'', thumb_position:'center center'});
  }
  const prev = kcData[index] || {};
  kcData[index] = {
    id:             index,
    title:          prev.title || '',
    file:           path,
    thumbnail:      prev.thumbnail || '',
    thumb_position: normalizeKcThumbPosition(prev.thumb_position)
  };
  await saveKcCards();
  renderCardMode();
